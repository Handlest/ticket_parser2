...
- **Источник кодов** — публичный location-сервис S7, тот же, что питает
  ...
  нового города.
- **Выбор кандидата и агрегатный код.** Берём ПЕРВОГО кандидата — S7 ранжирует
  ответ по релевантности к запросу. Для многопортовых городов первым идёт
  агрегатный `type="city"` код (напр. «Москва» → `MOW` = SVO/DME/VKO), он
  работает в deeplink (`AA1=MOW` проверено вживую), поэтому **один поиск
  покрывает все аэропорты** города — перебирать комбинации не нужно. Для
  однопортовых городов первым идёт сам аэропорт. Важно: НЕ предпочитать
  `type="city"` глобально — для «Улан-Удэ» (UUD, один аэропорт) это увело бы
  выбор на стоящий ниже «Улан-Батор» (ULN, многопортовый).
